...
from exercises.exercise_4_audit import build_graph as build_review_graph
...
with st.sidebar:
    ...
    for row in asyncio.run(list_recent_sessions()):
        label = f"{row['worst_risk']} · {row['pr_url'].split('/')[-1]} · {row['last_event'][:19]}"
        if st.button(label, key=f"recent-{row['thread_id']}"):
            st.session_state.thread_id = row["thread_id"]
            st.session_state.pr_url = row["pr_url"]
            st.session_state.interrupt_payload = None
            st.session_state.final = {
                "final_action": "loaded_from_history",
...
def render_escalation_card(payload: dict) -> dict | None:
    ...
    with st.form("escalation"):
        ...
        for idx, question in enumerate(payload.get("questions", []), start=1):
            answers[question] = st.text_input(f"Q{idx}. {question}", key=f"escalation_{idx}")
